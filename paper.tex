...
\DeclareMathOperator{\I}{\mathbb{1}}             % probability
...
\abstract{
The cover time of a graph is the number of steps a 
discrete random walk takes to visit every vertex at least once.
We begin by investigating the cover time of small and structured graphs but
quickly find that our precise strategy fails
for larger, unstructured ones.
To bound the cover time on arbitrary graphs,
...
complete graphs with self-edges and apply our analysis to an intuitive example.
}
...
Let $C_{i,j}=C_{j,i}$ be a non-negative weight assigned to the edge
...
\end{align}
Let the cover time random variable $\cov$ be the first time all vertices have been
...
of all $C_{x,y}$ where $x$ and $y$ share an edge,
and the probability $\pr_{x,y}$
...
Define $\pi_x$ as $C_x/C$ where $C=\sum_y C_y$.
We will show that $\pi$, the vector of $\pi_i$ where $i$
runs over all vertices, is the stationary distribution of the random walk.
Then $\pi_x$ must be the long term proportion of time spent in vertex $x$.
We first use our definitions to show that $\pi$ satisfies detailed balance,
\begin{align}
...
or detailed balance.
We next use detailed balance to show that $\pi$ is the stationary distribution.
Summing over $x$,
\begin{align}
\sum_x \pi_x \pr_{x,y} = \sum_x \pi_y \pr_{y,x} = \pi_y \sum_x \pr_{y,x}
= \pi_y . \nonumber
\end{align}
(We get the last equality because we know that when leaving vertex $y$
the random walk goes with probability 1 to some vertex $x$.)
We rely on \cite{Ka14} to show that 
$\pi \pr = \pi$ where $\pr$ is the matrix of transition probabilities
and how it follows that $\pi$ is the stationary distribution of the random walk.
Then $\pi_x$ is indeed the long term proportion of time spent in vertex $x$.

We now introduce voltage, current, and effective resistance.
Consider arbitrary $a$ and $b$ in $V(G)$.
This is equivalent to setting the voltage to be 1 at $a$ and 0 at $b$.
We may think of voltage $v_x$ as the probability starting
from $a$ we get to $x$ before $b$.
(We have not proved this yet but we will by the end of this subsection.)
With this interpretation
it makes sense that $v_a$, the probability we get to $a$ before $b$, is 1
(we are already at $a$) 
and $v_b$, the probability we get to $b$ before $a$, is 0
(we are again already in $a$).
We do not prove this but
...
We draw on \cite{Du11} and find that the cover time random variable
approaches a Poisson distribution as the number of vertices goes to infinity.
...
In principle, it is always possible to find the exact cover time
...
\begin{figure}[ht]
	\centering
		\begin{tikzpicture} [
			...
			\path[->] (C) edge (E);
			\path[<->] (D) edge (F);
			\path[->] (D) edge[bend left, distance=12mm] (H);
			\path[<->] (E) edge (G);
			\path[->] (E) edge[bend right, distance=15mm] (H);
			\path[->] (E) edge[loop above, distance=5mm] (E);
			\path[->] (F) edge (H);
			\path[->] (G) edge (H);
	\end{tikzpicture}
	\caption{First step analysis applied to a small graph with $n=4$.}\label{fig:example}
...
\begin{figure}[ht]
	...
	\end{tikzpicture}
    \caption{The complete graph with six vertices (and 15 edges).}\label{fig:complete}	
\end{figure}
...
from $i-1$ to $i$ distinct vertices.

We write the cover time in terms of $X_i$ and use linearity of expectation.
\begin{align}
\tcov&= \E[X_1+X_2+X_3+\cdots+X_{n}] = \sum_{i=1}^{n} \E[X_i] \nonumber
\end{align}

The random variable $X_i$ takes value $k$ when $k-1$ steps lead us to
already visited vertices and the $k^{th}$ step is 
to a previously unvisited vertex.
...
we go to a previously unvisited vertex is $(n-i+1)/(n-1)$.
This is because there are $n-i+1$ unvisited 
vertices and a total of $n-1$ adjacent vertices.
It follows that $\E[X_i] = (n-1)/(n-i+1)$.
Since we start at a vertex, $\E[X_1]=0$.
Then
\begin{align}
\tcov&=   0+\sum_{i=2}^{n} \frac{n-1}{n-i+1} \nonumber \\
&=  (n-1) \left(\frac{1}{n-1}+\frac{1}{n-2}+\cdots+1\right). \nonumber
\end{align}

A natural interpretation of the cover time on a complete graph is 
the coupon collector problem we introduced in \cref{sec:intro}.
There are $r$ distinct coupons one of which arrives in the mail everyday.
We want to know how many days until the collector has collected each coupon.
...
The difference between the coupon collector problem and the complete graph
is that we now have self-edges
whereas before we had to leave our current vertex at each step.
That is, we can now stay in the same vertex (provided the collector 
receives the same coupon two days in a row).
...
Then
\begin{align}
\tcov&=  r \left(\frac{1}{r-1}+\frac{1}{r-2}+\cdots+1\right). \nonumber
\end{align}
...
naturally follows that $e_1=e_{N-1}$.
...
line graph with $N+1$ vertices can be relabeled so that the left endpoint
...
The commute time between vertices $a$ and $b$ is the expected 
number of steps for a random walk starting in $a$ to visit $b$
and then return to $a$.
Following \cite{DS84}, we use the electrical network analysis in \cref{sec:electric} to 
...
\begin{figure}[ht]
	...
	\begin{tikzpicture}[scale = 2]
	    ...
	    \draw[edge] (6) -- (14);
	\end{tikzpicture}
	\caption{A binary tree with $k=3$ has $n=15$.}\label{fig:binary}
\end{figure}

Following \cite{LP17},
consider an unweighted, balanced binary tree as in \cref{fig:binary}
with a single root $\rho$ and $k$ levels.
...
There are $n = 1 + 2 + 4 + \cdots + 2^k = 2^{k+1} - 1$ vertices.

By \cref{eqn:commute}, the commute time between the root and a leaf $a$ is
\begin{align}
C R_{\rho, a} = 2 (n-1) k \nonumber.
\end{align}
Note that resistance adds in series so 
effective resistance is just the sum of the resistances along a path.
Then $\Rxy = i$ where $i$
is the length of the unique path between $x$ and $y$.
...
\begin{align}
\tcov &\leq 2(n-1)k\left(1 + \frac{1}{2} + \cdots + \frac{1}{n} \right) \nonumber \\
&\sim (2 - \frac{2}{n}) \cdot n k \log n 
\sim(2 - \frac{2}{n}) n k \log 2^{k+1} -1 \nonumber \\
&\sim(2  + o(1)) \cdot n k^2 \log 2 \nonumber
\end{align}

We now use the Matthews Lower Bound.
We want to choose a set $A \subseteq V(G)$ with a large hitting time between any
pair of vertices.
...
\begin{align}
\tcov &\geq 2 (n-1)(k-h+1)\left(1 + \frac{1}{2} + \cdots + \frac{1}{2^h -1} \right) \nonumber \\
&\geq (2 + o(1)) \cdot n (k-h) h \log 2 \nonumber.
\end{align}
...
A tree is a connected graph with no cycles (i.e. there is exactly one path
between every pair of points.)
A spanning tree $\T$ for a connected graph $G$ is a tree with $V(\T) = V(G)$
and $E(\T) \leq E(G)$.
\cref{fig:span} shows an example spanning tree
where the bold edges indicate the edges in $E(\T)$
...
\end{align}
The second equality holds because we distribute terms.
The third equality holds because $\I_m \I_k = \I_{m \cap k}$
and the expectation of an indicator random variable is the event's probability.
...
&= n(n-1) \left[ \pr(A_k \cap A_m) - P(A_k)P(A_m) \right] + 
n \left[ \pr(A_k \cap A_m) - P(A_k)P(A_m) \right] \nonumber \\
&= n(n-1) \left[ (1-2/n)^r - (1-1/n)^{2r} \right] + 
n \left[ (1-1/n)^r  - (1-1/n)^{2r} \right]. \nonumber
\end{align}
...
Thus we conclude that $N_n/n$ converges in probability to $e^{-c}$. 
...
We can use this to intuite our latest result: the proportion of unvisited vertices
approaches $e^{-c}$.
...
If $ne^{-r/n} \rightarrow \lambda \in [0, \infty)$ as $n \rightarrow \infty$,
then the number of unvisited
vertices approaches a Poisson distribution with mean $\lambda$.
\end{theorem}

\begin{proof}
Observe that the chance a particular set of $k$ vertices are unvisited is
\begin{align}
(1-k/n)^r \nonumber.
\end{align}
...
If we consider the location of the $m$ unvisited vertices,
and multiply probability that those $m$ vertices are unvisited
and the rest are visited,
\begin{align}
...
&\geq n^m \left(1 - \frac{m}{n}\right)^r \left(1- \frac{m}{n}\right)^m
\frac{1}{m!}. \nonumber
\end{align}

As $n \rightarrow \infty$, $\left(1-\frac{m}{n}\right)^m \rightarrow 1$
and $1/m!$ is constant.
We use the Taylor series to bound the rest.
(The following equality is valid for $-1 \leq t \leq 1$
but we only need $0 \leq t \leq 1$ because $m/n \rightarrow 0$
and $m,n$ are counts of vertices with $m\leq n$.)
We apply
\begin{align}
...
\begin{align}
ne^{-r/n} &\rightarrow \lambda \nonumber\\
\log n - \frac{r}{n} &\rightarrow \log \lambda \nonumber\\
\frac{(n\log n - n\log\lambda) - r}{n} &\rightarrow 0 \nonumber
\end{align}
then
\begin{align}
...
\begin{align}
p_0(r,n) \rightarrow \sum_{k=0}^\infty (-1)^k \lambda^k /k! = e^{-\lambda}. \nonumber
\end{align}
...
(with $e^x$ in the role of $\lambda$),
\begin{align}
...
\node (title) at (1, 12) {Cover Times of the Birthday Problem (1000 trials)};
\node[rotate=90] (ylabel) at (-15,2) {$\pr(\cov \leq r)$};
\node (xlabel) at (0,-9) {$r$};
\end{tikzpicture}
\caption{The cumulative distribution of the birthday problem. The red line denotes the
empirical distribution from 1000 trials. The blue line denotes the distribution from
\cref{eqn:dist}. (Our code to generate the figure appears in 
\cref{app:birthday}.)}\label{fig:birthday}
\end{figure}
...
\newpage
\section{Appendix}
\subsection{Birthday Problem R Code}\label{app:birthday}
