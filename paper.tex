...
\todo{write general introduction to cover times}
"white screen" problem
numerous applications
many mathematical tools

\subsection{Motivation}
\todo{write about applications}
...
Rely on \cite{BH94}
Always possible to find cover time but it gets hard as the graph gets bigger
Structure helps:
Complete graph,
Line graph,
Simple cycle (extended line graph)

In lieu of structure, we want to work with arbitrary graphs
Rely on \cite{DS84} for commute time relationship
Rely on \cite{LP17}
Matthews method is intuitive approach that uses same ideas as complete graph
Matthews method applied to binary tree with commute time relationship
Use \cite{AF14}
Explore the idea of a spanning tree and use it to bound time with commute identity

So far we've only worked with expected time
\cite{Du11}
Explore what happens for the variable and it's distribution

\todo{outline of each section and subsections}
...
\section{Cover Time Distributions}
...
There appears to be a critical stage where it quickly becomes very likely
that all birthdays have been represented.
\cref{fig:birthday} shows the cumulative distribution of the birthday problem.
