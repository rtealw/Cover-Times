...
Let the cover time $\cov$ be the until all vertices have been
...
The black vertex indicates the current vertex of the random walk
and the empty vertices indicate the visited nodes.
...
It follows from our combinatorial analysis that the number of states
...
\begin{figure}[ht]
	\centering
		\begin{tikzpicture} [
			...
			\path[->] (g) edge (done);
	\end{tikzpicture}
	\caption{First step analysis applied to an asymmetric graph with $n=4$.}\label{fig:example}
\end{figure}
